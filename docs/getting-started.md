...
3. Click any stratum (stratum) to expand it
4. Explore substrates and constructs inside each stratum
...
The central canvas shows the {a}OS reference stack — 7 horizontal strata stacked vertically from S7 (top, user-facing) to S1 (bottom, infrastructure).

Each stratum has:

- A **color-coded indicator** on the left edge
- A **stratum number** (S1–S7)
- A **name** describing its domain
- A **boundary question** — the key question that stratum answers
...
1. Try expanding S4 (Orchestration) and compare the products that overlap there
